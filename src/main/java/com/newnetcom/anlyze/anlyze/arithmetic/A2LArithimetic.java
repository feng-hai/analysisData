...
public class A2LArithimetic implements ILoadData {
	...
	public void setPairValue(byte[] content) {
			...
			if (!isNum) {
				...
				switch (num) {
				case 1:
					bean.setValue(String.valueOf(ByteUtils.formatDouble((content[0]&0xff) * resolving + bean.getOffset(),nums)));
					break;
				case 2:
					bean.setValue(
							String.valueOf(ByteUtils.formatDouble((ByteUtils.getShortForLarge(content, 0)&0xffff) * resolving + bean.getOffset(),nums)));
					break;
				case 3:
					bean.setValue(String
							.valueOf(ByteUtils.formatDouble((ByteUtils.getThreeByte(content, 0)) * resolving + bean.getOffset(),nums)));
					break;
				case 4:
					bean.setValue(
							String.valueOf(ByteUtils.formatDouble((ByteUtils.getIntForLarge(content, 0)&0xffffffff) * resolving + bean.getOffset(),nums)));
					break;
				}
			} else {
				...
				case 2:
					bean.setValue(
							String.valueOf((ByteUtils.getShortForLarge(content, 0)&0xffff) * resolving + (int)bean.getOffset()));
					break;
				case 3:
					bean.setValue(String
							.valueOf((ByteUtils.getThreeByte(content, 0)) * resolving + (int)bean.getOffset()));
					break;
				case 4:
					bean.setValue(
							String.valueOf((ByteUtils.getIntForLarge(content, 0)&0xffffffff) * resolving + (int)bean.getOffset()));
					break;
				}
			}
