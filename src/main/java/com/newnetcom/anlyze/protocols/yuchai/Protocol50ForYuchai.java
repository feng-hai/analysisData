...
* @version        
*/
public class Protocol50ForYuchai {
	private static final Logger logger = LoggerFactory.getLogger(Protocol50ForYuchai.class);
	...
	* @param contents 
	*/
	public Protocol50ForYuchai(byte[] contents) {
	...
	*/
	private void anlyze() {
		...
		int terminalCommandLength = ByteUtils.getShortForLarge(this.contents, 5);
...
//		canNum = this.contents[27];
		...
		canNum=terminalCommandLength/15;
		for (int i = 0; i < canNum; i++) {
			byte[] key = ByteUtils.getSubBytes(this.contents, i * 15+7, 4);
			byte[] value = ByteUtils.getSubBytes(this.contents,i * 15+7+4+1,8);
