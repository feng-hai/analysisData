...
//                conf.set("hbase.zookeeper.property.clientPort", ConfigUtil.getInstance().getConfigVal("zkport", ConstantProperties.COMMON_PROP));
				...
				conf.set("hbase.rootdir", "hdfs://"+PropertyResource.getInstance().getProperties().get("namenode")+":9000/hbase");
...
//         System.out.println("---------- " + conn.hashCode());
    ...
    /**
     * 获得现已有的快照
     * @param snapshotNameRegex 正则过滤表达�?
     * @return
     * @throws IOException
     */
	...
	/**
	 * 分页�?索表数据�?<br>
	 * （如果在创建表时为此表指定了非默认的命名空间，则�?拼写上命名空间名称，格式为�?�namespace:tablename】）�?
	 * @param tableName 表名�?(*)�?
	 * @param startRowKey 起始行键(可以为空，如果为空，则从表中第一行开始检�?)�?
	 * @param endRowKey 结束行键(可以为空)�?
	 * @param filterList �?索条件过滤器集合(不包含分页过滤器；可以为�?)�?
	 * @param maxVersions 指定�?大版本数【如果为�?大整数�?�，则检索所有版本；如果为最小整数�?�，则检索最新版本；否则只检索指定的版本数�?��??
		...
		final BufferedMutator.ExceptionListener listener = new BufferedMutator.ExceptionListener() {
			...
			public void onException(RetriesExhaustedWithDetailsException e, BufferedMutator mutator) {
				...
				for (int i = 0; i < e.getNumExceptions(); i++) {
					...
					logger.error("Failed to sent put " + e.getRow(i) + ".",e);
				}
			}
		};
